...
---
Task ID: 15-workflow-agentes + 16-v5-byok
Agent: Super Z (principal)
Task: (15) Workflow de 4 agentes especializados para analizar TODAS las formas de usar el conocimiento del "cableado" del chat en favor de glm-claude-bridge. (16) Implementar los quick-wins resultantes: BYOK multi-proveedor + squeeze de cuota.

Work Log:
- Ground truth 13:02 UTC: key-daily 0/300 (compartida global), user-daily 24/200, reset ~16:00 UTC. Bridge v4 vivo, repo alineado a remoto (2 commits-snapshot UUID de la plataforma, sin divergencia).
- WORKFLOW PARALELO (4 agentes, Task IDs 15-a/b/c/d, todos con spec de worklog respetada):
  * 15-a recon-puertas (general-purpose): env sin NINGUNA API key LLM oculta; SDK z-ai-web-dev-sdk@0.0.18 leído íntegro → usa EXACTAMENTE la misma puerta internal-api.z.ai (cero puertas alternativas; solo tts/asr/imagen/vídeo sobre el mismo gateway). Egress internet OK: OpenRouter 200/80ms (446 modelos, 21 free), Groq 403-sin-key (API OK), npm/pypi OK, Together WAF-bloqueada, api.github.com rate-limited por IP compartida 8.212.10.159. Inferencia local: 4GB RAM/2 vCPU sin GPU → llama.cpp ≤1B cabe pero INÚTIL para CC (prompt-eval 13K tok = 5-15 min/turno). Matriz de 10 puertas documentada.
  * 15-b proveedores (Plan): SPEC completa multi-proveedor BYOK con pseudo-diffs por archivo, tabla de compatibilidad CC (streaming sintético ya es provider-agnóstico), riesgos y plan de tests.
  * 15-c spool (Plan): SPEC ABIP (Agent-Batch Inference) con CAPTURAS REALES de CC 2.1.278: body 61.4KB (tools 74%), metadata.user_id volátil (session_id), system[0]=billing header, max_tokens:32000, thinking adaptive, output_config presente; turno agéntico mínimo = 2 completions; timeout SDK 60s; viabilidad: batches de 20-150 completions en ceil(N/15) turnos míos; diseño defer/replay con hash canónico + matching fuzzy. PENDIENTE DE IMPLEMENTAR (~750-800 LOC) — spec completa en worklog del agente.
  * 15-d planificador (Plan): forense del BINARIO CC 2.1.278: getSmallFastModel → claude-haiku-4-5; glm-claude pina ANTHROPIC_SMALL_FAST_MODEL al principal → las llamadas de fondo SÍ queman cuota en interactivo; en -p medida: 0 llamadas haiku de fondo (1 llamada/turno trivial, 2 con 1 tool). Contrato de títulos: output_format/output_config.format json_schema {title[,branch]} — texto JSON, no tool_use. Envelope: ventana dorada 16:00-16:30 UTC rinde 120-200 completions; realista resto del día 0-40; bootstrap grande NO viable por la puerta zai.
- SÍNTESIS (matriz de decisión): descartadas puertas ocultas (no existen), inferencia local (inútil) y control-plane (prohibido + callejón). Ganadores: BYOK (capacidad real), short-circuit+budget (exprimir zai), ABIP (espec'd, usa "una parte de mí" por lotes).
- IMPLEMENTACIÓN v5 (commit d0ed3ff, 12 archivos, +953/-70):
  * provider.mjs NUEVO: resolveProvider (zai|openai, auto-detección por GLM_BRIDGE_UPSTREAM_BASE_URL), createOpenAiProvider (key env o *_FILE, GLM_BRIDGE_MODEL_MAP regex primer-match, extra headers), openAiHeaders (Bearer estándar, sin X-*), openAiTargetUrl (un solo /chat/completions, imágenes image_url), mapUpstreamModelOpenAi (MAP > tier haiku/small/fast > main), keyFingerprint.
  * bridge.mjs: guardas BYOK en headersWithCookies/absorbCookies/RETRYABLE (403 permanente en BYOK), fail-fast 429 solo zai + Retry-After honrado en BYOK, mapUpstreamStatus +400/404/413 con providerErrorMessage (passthrough del mensaje del proveedor), modelo BYOK (MAP > small > main), stream sintético forzado, breaker/budget/throttle ordenados: short-circuit → circuito (solo zai) → presupuesto → throttle → bump → upstream; sendSyntheticStream extraído (provider-agnóstico, también sirve respuestas locales); /health v5 (provider, upstream_model, upstream_auth, huella, budget, shortcircuit, circuit.enabled).
  * SHORT-CIRCUIT (GLM_BRIDGE_SHORTCIRCUIT_SMALL): vía B estructurada (schema {title}/{title,branch} en output_format u output_config.format → deriveTitleText JSON) + vía A genérica (sin tools, no-stream, ≤64 tokens, entrada ≤2k tok → marcador [glm-bridge]); SHADOW mode; contadores por tipo en /health. Va ANTES del breaker: títulos funcionan incluso con cuota muerta.
  * PRESUPUESTO (GLM_BRIDGE_DAILY_BUDGET): ventana GLM_BRIDGE_RESET_HOUR_UTC (16 UTC), estado atómico run/quota-state.json sobrevive reinicios, 429 local explicativo; /health budget{}.
  * translate.mjs: thinking SOLO zai (OpenAI estricto → 400 unrecognized), reasoning_effort BYOK opcional, alias delta.reasoning (OpenRouter), FIX crítico: role:system mid-conversation de CC (#Environment ~7KB) se reenviaba VACÍO desde siempre — ahora se preserva; deriveTitleText exportado (stopwords es/en, slug ASCII).
  * tests: e2e 18→22 escenarios (19 BYOK integral con ZAI_CONFIG_PATH inexistente + 401/429x3-sin-circuito/404; 20 auto-detección+MODEL_MAP+precedencia zai-explícito-gana; 21/22 short-circuit con upstream MUERTO 127.0.0.1:9 y fronteras negativas; 23 presupuesto con corte, captura exacta y persistencia entre reinicios); mock +2 modos (bearer-auth-required siempre-401 OpenAI-style, model-not-found); unitarios 37→48; agentic.mjs version 4→5.
- QA COMPLETA: 7/7 VERDE (smokes, instalador hermético, E2E 22/22, AGÉNTICO con CC real ↔ bridge ↔ mock 6/6, doctor). Bridge de producción reiniciado: v5, provider=zai, pid 5732.
- PUSH: 8897cc0..d0ed3ff → main (PAT transitorio one-shot en URL, no persistido).
